            ...
            {GRADES_DATA.map((row, i) => (
              <div key={i} className="glass-card rounded-2xl p-5">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {row.grades.map((g, j) => (
                    <div key={j} className="w-9 h-9 rounded-xl flex items-center justify-center font-display text-base font-bold"
          ...
          <div className="space-y-3 animate-fade-in">
            {tasks.length === 0 && (
              <div className="text-center py-12 text-muted-foreground font-body">
                ...
                <p>Заданий пока нет. Администратор добавит их в панели управления.</p>
              </div>
            )}
            {tasks.map(task => {
              const done = doneTasks.has(task.id);
              return (
                <div key={task.id} className="glass-card rounded-2xl p-5 flex items-start gap-4" style={{ opacity: done ? 0.6 : 1 }}>
                  <button onClick={() => toggleTask(task.id)} className="mt-0.5 flex-shrink-0 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all"
                    style={{ borderColor: done ? '#ff6b1a' : 'rgba(255,107,26,0.3)', background: done ? '#ff6b1a' : 'transparent' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-body text-white" style={{ textDecoration: done ? "line-through" : "none" }}>{task.task}</p>
